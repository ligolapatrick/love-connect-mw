...
app.use(
  session({
    store: new SQLiteStore({ db: 'sessions.sqlite', dir: './' }), // Store sessions in SQLite
    secret: '3#dF!9xS2ZjD%VuWqRtL7&aYH6PbNvQ8!', // Replace with a secure secret
    resave: false,
...
const User = sequelize.define('User', {
  id: {
    ...
    primaryKey: true,
    autoIncrement: true // SQLite supports auto-increment for INTEGER type
  },
  username: {
    type: DataTypes.STRING,
    allowNull: false,
    unique: true // Ensures uniqueness in SQLite
  },
  password: {
    type: DataTypes.STRING,
    allowNull: false
  ...
  fullPhoneNumber: {
    type: DataTypes.STRING,
    allowNull: false,
    unique: true // Enforces unique phone numbers
  },
  profilePicture: {
    type: DataTypes.STRING,
    allowNull: true
  ...
  online: {
    ...
    allowNull: true,
    defaultValue: false // SQLite handles BOOLEAN as INTEGER (0/1)
  },
  bio: {
    type: DataTypes.TEXT, // SQLite supports TEXT for long strings
    allowNull: true
  },
  mood: {
    type: DataTypes.STRING,
  ...
  interests: {
    type: DataTypes.TEXT, // SQLite works well with TEXT
    allowNull: true,
    get() {
  ...
  badges: {
    type: DataTypes.JSON, // SQLite supports JSON for structured data
    allowNull: true,
  ...
  indexes: [
    { fields: ['username'] }, // Adds index for username
    { fields: ['fullPhoneNumber'] } // Adds index for phone number
  ]
});
...
const Message = sequelize.define('Message', {
  id: {
    type: DataTypes.INTEGER,
    primaryKey: true,
    autoIncrement: true // SQLite supports auto-incrementing for INTEGER type
  },
  fromUserId: {
    type: DataTypes.INTEGER,
    allowNull: false
  ...
  content: {
    type: DataTypes.TEXT, // SQLite supports TEXT type for longer strings
    allowNull: false,
    validate: {
      len: [1, 500] // Enforces length validation to avoid overly long content
    }
  },
  timestamp: {
    type: DataTypes.DATE,
    allowNull: false,
    defaultValue: Sequelize.NOW // Sets default timestamp
  },
  read: {
    type: DataTypes.BOOLEAN,
    defaultValue: false // SQLite handles BOOLEAN type as INTEGER (0 or 1)
  },
  favorite: {
    type: DataTypes.BOOLEAN,
    defaultValue: false // Similar to `read`, uses INTEGER for Boolean values
  },
  deletedAt: {
    type: DataTypes.DATE, // Paranoid feature relies on this column for soft deletion
    allowNull: true
  }
}, {
  paranoid: true, // Enables soft delete functionality using `deletedAt`
  indexes: [
    { fields: ['fromUserId'] }, // SQLite supports indexing on columns
    { fields: ['toUserId'] },
    { fields: ['fromUserId', 'toUserId'] } // Composite index for sender-receiver conversations
  ]
});
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    const existingPhoneUser = await User.findOne({ where: { fullPhoneNumber } });
...
app.get('/api/me', requireLogin, async (req, res) => {
  try {
    const me = await User.findByPk(req.session.userId, {
      attributes: ['id', 'username', 'profilePicture', 'region']
    });
...
Comment.hasMany(Comment, { as: 'Replies', foreignKey: 'parentCommentId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });
Comment.belongsTo(Comment, { as: 'Parent', foreignKey: 'parentCommentId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });

// Post-to-Likes/Dislikes Relationships
Post.hasMany(PostLike, { foreignKey: 'postId', as: 'PostLikes', onDelete: 'CASCADE', onUpdate: 'CASCADE' });
PostLike.belongsTo(Post, { foreignKey: 'postId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });

Post.hasMany(PostDislike, { foreignKey: 'postId', as: 'PostDislikes', onDelete: 'CASCADE', onUpdate: 'CASCADE' });
PostDislike.belongsTo(Post, { foreignKey: 'postId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });

// User-to-Likes/Dislikes Relationships
PostLike.belongsTo(User, { foreignKey: 'userId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });
PostDislike.belongsTo(User, { foreignKey: 'userId', onDelete: 'CASCADE', onUpdate: 'CASCADE' });
...
const BlockedUser = sequelize.define('BlockedUser', {
  ...
  userId: {
    ...
    onDelete: 'CASCADE', // Ensure that blocks are deleted if the user is deleted
    onUpdate: 'CASCADE'
  },
  blockedUserId: {
    type: DataTypes.INTEGER,
...
Like.belongsTo(User, { foreignKey: 'likedUserId' });

User.hasMany(Dislike, { foreignKey: 'userId' });
Dislike.belongsTo(User, { foreignKey: 'dislikedUserId' });
...
app.get('/api/matches-base', async (req, res) => {
  ...
  try {
    const user = await User.findByPk(userId); // Find the logged-in user
    if (!user) {
    ...
    if (!oppositeGender) {
      return res.status(400).send('User gender is not set or invalid');
    }
    ...
    const matches = await User.findAll({
      ...
      order: [['createdAt', 'DESC']] // Sort by newest users
    });

    // Parse the interests field and return matches
    ...
    res.json(parsedMatches); // Send matches as response
  } catch (error) {
